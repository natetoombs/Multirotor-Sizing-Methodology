...
%
% Changes: 
%   -Allow for props 2x the mass of the est (line 73)
%   -Allow for motors 2x the mass of the est (line 145)
%% ------------------------------------------------------------------------
...
PropDiameter_Max = 11; % inch, max. propeller diameter
SafetyFactor = 1.1; % [1-2], arbitrary safety parameter
AcceptedTypes = {'MR' 'E-3' 'E-4' 'E' 'F'}; % preferred propeller series
                  % E	Electric
...
BattCapacity = 6600; % mAh, battery capacity
...
%% Mass data [g]
mass_Frame = 475; % S500 + Landding Skid
mass_FC = 21 + 300; % FC + NUC
mass_FC_GPS = 13;
mass_FC_CurrentSensor = 15;
mass_Receiver = 2; % FrSky R-XSR 2.4GHz 16CH ACCST
mass_Motor_Est = 80; % T Motor MN3110
mass_ESC_Est = 32; % 30A ESC
mass_Propeller_Est = 15; % HQProp 12x4.5 Props - 18g
mass_Payload = 300 + 200 + 0; % Camera, other, payload
mass_Battery = 660; % Lumenier 6600mAh 4s 35c
...
propList_considered_byMass = propList(cellfun(@(x) x<=mass_Propeller_Est*2, propList(:,5)), :); % filtering propellers based on mass
...
motorList = load_motorList(BattCellNo*BattCellVoltage, operatingPoints{temp_propChosen_pos,2}(1), operatingPoints{temp_propChosen_pos,2}(3),...
    ...
    mass_Motor_Est*2); % loading motor set with operating points
