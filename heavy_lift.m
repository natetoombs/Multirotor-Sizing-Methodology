...
propList = load_propListx(brand);
propList_considered = [];

% Filter by size, data availability:
for ii = 1:size(propList,2)
    if propList(ii).diameter >= PropDiameter_Min && propList(ii).diameter <= PropDiameter_Max && propList(ii).data == 1
...
for ii = 1:consideredNo
    % TRUE/FALSE for plot
    propPerf(ii) = {load_propPerfx(propList_considered(ii))}; % loading propeller static performance data
end
...
for ii = 1:consideredNo
    speedHover = interp1(propPerf{ii}(2:end,2), propPerf{ii}(2:end,1), thrustHover_Est, 'linear', 'extrap'); % obtaining propeller speed at hover from required thrust for hover, Eq. (11)
    speedMax = interp1(propPerf{ii}(2:end,2), propPerf{ii}(2:end,1), thrustMax_Est, 'linear', 'extrap'); % obtaining propeller speed at WOT from estimated thrust at WOT , Eq. (11)
