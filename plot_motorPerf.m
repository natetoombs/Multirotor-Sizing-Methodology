...
%% ------------------------------------------------------------------------
%     conn = sqlite('/home/nathan/Documents/MATLAB/AFRL/Multirotor-Sizing-Methodology/DCbase.dcd'); % connect to the database
...
%     close(conn); % close connection to database
    ...
    % motors gives: motor ID, rec. ESC, current_max, Rm, kV, mass
    % mdata gives: performance
    motors = array2table(xlsread('Motors.xlsx'));
    mdata = array2table(xlsread('MData.xlsx'));
    esc = array2table(xlsread('ESC.xlsx'));
    
motors = motors(find(motors{:,1}==motorChosen{1}),:);
esc_id = motors{1,6}; % obtain ID of ESC recommended for the motor
performance = mdata([mdata{:,3}] == motorChosen{1},:); % look for test data for the motor with given ID

current_maximum = motors{1,13}; % obtain maximum motor current
...
Rm = 2*motors{1,18}; % read motor windings resitance
kV = motors{1,17}; % read motor KV rating
