...
figure; hold on;
temp_plot_points = [];
temp_RPMRange = 1000:100:max([propChosen.rpm_max]);
...
hold off; grid;
legend (propList_considered(:).name, 'Location', 'NorthWest');
...
plot(temp_RPMRange(15:end), motorChosen.Pmax./(temp_RPMRange(15:end)*2*pi/60));
...
hold off; grid;
legend ({[propChosen.name ' propeller'] ['Constant power - ' num2str(round(motorChosen.Pmax)) ' W']}, 'Location', 'NorthWest');
ylabel('Torque (Nm)');
xlabel('Speed (RPM)');
...
title([num2str(batterySpecification(1)) 'S ' num2str(ceil(batterySpecification(2))) 'C ' num2str(ceil(batterySpecification(3)/100)*100) 'mAh - energy usage at hover']);
...
title([num2str(batterySpecification(1)) 'S ' num2str(ceil(batterySpecification(2))) 'C ' num2str(ceil(batterySpecification(3)/100)*100) 'mAh - energy usage at 100% throttle']);